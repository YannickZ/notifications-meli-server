...
const db = mongosking.db(mongoUri, {
  native_parser: true
})
  ...
  users.findOne({
    ...
    if (!user) {
      ...
      userGroup.findOne({
        ...
        notifications.find({
          "type": {
            $in: user_group.notification_tags
          }
          ...
          let filteredNotifications = notifications.filter(obj => {
            return !user.read_notifications.includes(obj._id.toString()) && user.subscribed_applications.includes(obj.application)
          })
